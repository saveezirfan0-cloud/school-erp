...
// ─── Column mapping templates for each Manager.io export ───
const TEMPLATES = {
  students: {
    ...
    columnMap: {
      ...
      "parentPhone": ["phone", "mobile", "telephone", "phone number", "mobile number"],
      ...
      "monthlyFee": ["balance", "amount", "fee", "monthly fee", "credit limit"],
    },
    transform: (row) => ({
      ...row,
      recurringFee: false,
  ...
  employees: {
    ...
    columnMap: {
      ...
      "salary": ["salary", "wage", "pay", "monthly salary", "basic salary"],
      "joinDate": ["start date", "hire date", "join date", "date hired"],
    },
    transform: (row) => ({
      ...row,
  ...
  expenses: {
    ...
    bg: "#fef2f2",
    collection: "expenses",
    description: "Export from Manager.io: Expense Claims or Purchase Invoices → Export",
    requiredColumns: ["description"],
    columnMap: {
      "description": ["description", "memo", "details", "particulars", "narration", "item"],
      "amount": ["total", "amount", "net amount", "total amount"],
      "date": ["date", "expense date", "transaction date", "issue date"],
  ...
  accounts: {
    ...
    bg: "#f5eaec",
    collection: "accounts",
    description: "Export from Manager.io: Chart of Accounts tab → click Export",
    requiredColumns: ["name"],
    columnMap: {
      ...
      "balance": ["balance", "opening balance", "debit balance", "credit balance"],
  ...
  payments: {
    ...
    bg: "#fffbeb",
    collection: "payments",
    description: "Export from Manager.io: Bank Accounts or Cash Accounts → Transactions",
    requiredColumns: ["amount"],
    columnMap: {
      "description": ["description", "memo", "narration", "particulars", "details"],
    ...
    transform: (row) => ({
      ...row,
      type: row.type?.toLowerCase()?.includes("debit") || row.type?.toLowerCase()?.includes("out") ? "cash_out" : "cash_in",
      branchId: "",
      createdAt: serverTimestamp(),
    })
  }
...
// ─── Map columns from file headers to our field names ───
function mapColumns(rows, columnMap) {
  ...
  for (const [field, aliases] of Object.entries(columnMap)) {
    for (const alias of aliases) {
      const found = fileHeaders.find(h => h === alias.toLowerCase() || h.includes(alias.toLowerCase()));
      if (found) {
        const originalHeader = Object.keys(rows[0]).find(h => h.toLowerCase().trim() === found);
...
function downloadTemplate(type) {
  ...
  const example = {
    students: ["Ahmad Khan", "ahmad@gmail.com", "+923001234567", "Mr. Khan", "House 5 Street 3", "Grade 5", "STU-001", "5000"],
  ...
  const ws = XLSX.utils.aoa_to_sheet([headers, example[type]]);
  ...
  const handleImport = async () => {
    ...
    for (const row of mappedData) {
      try {
        ...
        if (!hasRequired) {
          failed++;
          errors.push(`Skipped row — missing required field: ${template.requiredColumns.join(", ")}`);
          continue;
        }
    ...
    <div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: 14, marginTop: 2 }}>
          Import your existing data from Manager.io, Excel, or CSV files
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 10, marginBottom: 24 }}>
        {Object.entries(TEMPLATES).map(([key, t]) => (
          <button key={key} onClick={() => { setActiveType(key); reset(); }}
            style={{
              padding: "12px 14px", borderRadius: 10, border: activeType === key ? `2px solid ${t.color}` : "1px solid var(--border)",
              cursor: "pointer", background: activeType === key ? t.bg : "white",
              textAlign: "left", transition: "all 0.15s"
            }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: activeType === key ? t.color : "#1e293b", marginBottom: 2 }}>{t.label}</div>
        ...
        {[
          ...
          <React.Fragment key={n}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: step === n ? 600 : 400, color: step === n ? template.color : "var(--text-muted)", whiteSpace: "nowrap" }}>
                {label}
              </span>
            </div>
            {i < 2 && <div style={{ flex: 1, height: 2, background: step > n ? template.color : "#e2e8f0", maxWidth: 60 }} />}
          </React.Fragment>
        ))}
      </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 13, color: "var(--text-muted)", lineHeight: 1.7 }}>
              {template.description}. Then upload the file below.
              Supported formats: <strong>.xlsx</strong>, <strong>.xls</strong>, <strong>.csv</strong>
            </div>
            <div style={{ marginTop: 10, fontSize: 13, color: "var(--text-muted)" }}>
              Required columns: {template.requiredColumns.map(c => (
                <span key={c} style={{ padding: "2px 8px", background: template.bg, color: template.color, borderRadius: 4, fontSize: 12, fontWeight: 600, marginRight: 4 }}>{c}</span>
              ))}
            </div>
          </div>
      ...
      {step === 1 && (
        ...
        >
          <div style={{ width: 56, height: 56, borderRadius: "50%", background: template.bg, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
      ...
      {step === 2 && mappedData.length > 0 && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginBottom: 20 }}>
            {[
              { label: "Rows found", value: mappedData.length, color: template.color, bg: template.bg },
              { label: "File name", value: file?.name?.slice(0, 20) + (file?.name?.length > 20 ? "..." : ""), color: "#475569", bg: "#f1f5f9" },
              { label: "Fields mapped", value: Object.keys(template.columnMap).length, color: "#10b981", bg: "#ecfdf5" },
          ...
          <div style={{ background: "white", borderRadius: 12, border: "1px solid var(--border)", overflow: "hidden", marginBottom: 20 }}>
            <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontWeight: 600, fontSize: 14 }}>Preview — first {Math.min(5, mappedData.length)} of {mappedData.length} rows</span>
            </div>
          ...
          <div style={{ background: "#fffbeb", borderRadius: 10, padding: 14, border: "1px solid #fde68a", marginBottom: 20, fontSize: 13, color: "#92400e" }}>
            <strong>Note:</strong> The importer automatically detects column names from your file. If some fields show "—" in the preview, those columns were not found in your file. You can still proceed — missing fields will be left blank.
          </div>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            ...
            <button onClick={handleImport} disabled={importing}
              style={{ flex: 1, padding: "11px", background: importing ? "#c4a0a8" : template.color, color: "white", border: "none", borderRadius: 8, cursor: importing ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              {importing ? (
                <><RefreshCw size={16} style={{ animation: "spin 0.8s linear infinite" }} /> Importing {mappedData.length} records...</>
              ) : (
                <><Upload size={16} /> Import {mappedData.length} {template.label} Records</>
              )}
            </button>
          </div>
      ...
      {step === 3 && results && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 24 }}>
            {[
              { label: "Total Processed", value: results.total, color: "#475569", bg: "#f1f5f9" },
              { label: "Successfully Imported", value: results.success, color: "#10b981", bg: "#ecfdf5" },
              { label: "Failed / Skipped", value: results.failed, color: results.failed > 0 ? "#ef4444" : "#10b981", bg: results.failed > 0 ? "#fef2f2" : "#ecfdf5" },
            ].map(({ label, value, color, bg }) => (
            ...
            <div style={{ background: "#ecfdf5", borderRadius: 12, padding: 20, border: "1px solid #bbf7d0", marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: "#047857", marginTop: 2 }}>
                  {results.success} {template.label} records have been added to your ZMI system.
                  Go to the {template.label} page to view them.
                </div>
              </div>
            </div>
          )}
          ...
          {results.errors.length > 0 && (
            <div style={{ background: "white", borderRadius: 12, border: "1px solid var(--border)", overflow: "hidden", marginBottom: 16 }}>
              <button
                onClick={() => setExpandedSections(p => ({ ...p, errors: !p.errors }))}
          ...
          <div style={{ display: "flex", gap: 12 }}>
            ...
            <button onClick={() => { setActiveType(Object.keys(TEMPLATES)[(Object.keys(TEMPLATES).indexOf(activeType) + 1) % Object.keys(TEMPLATES).length]); reset(); }}
...
### Commit and push
```
add Manager.io data import system